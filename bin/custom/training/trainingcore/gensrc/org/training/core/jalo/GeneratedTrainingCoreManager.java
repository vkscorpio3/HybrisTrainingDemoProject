...
import org.training.core.jalo.MyTraining;
...
public abstract class GeneratedTrainingCoreManager extends Extension
{
	...
	public MyTraining createMyTraining(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.MYTRAINING );
			return (MyTraining)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating MyTraining : "+e.getMessage(), 0 );
		}
	}
	
	public MyTraining createMyTraining(final Map attributeValues)
	{
		return createMyTraining( getSession().getSessionContext(), attributeValues );
	}
